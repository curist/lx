...
- `Date.time() -> number` Current unix timestamp (seconds).
- `Date.format(unix: number, format?: string) -> string` Format unix timestamp.
- `Date.parse(s: string, format: string) -> number` Parse a date string with the given format.
